...
export default class Login extends Component {
    ...
    componentDidMount = async () => {
        ...
        try {
            ...
            var seller = await this.state.propertyInstance.methods.isSeller(currentAddress).call();
            console.log(seller);
            this.setState({ seller: seller });
            var buyer = await this.state.propertyInstance.methods.isBuyer(currentAddress).call();
            console.log(buyer);
            this.setState({ buyer: buyer });
            var propertyInspector = await this.state.propertyInstance.methods.ispropertyInspector(currentAddress).call();
            console.log(propertyInspector);
            this.setState({ propertyInspector: propertyInspector });
    ...
    render() {
        if (this.state.seller || this.state.buyer || this.state.propertyInspector) {
            return (
                ...
                <div className="bodyC">
                    <div className="img-wrapper">
                        ...
                        <div className="wine-text-container">
                            <div className="site-title wood-text">property Registry</div>
                        </div>
                    </div>
                    <div className="auth-wrapper">
                        <div className="auth-inner">
                            ...
                            <br/><Button href="/LI/LIdashboard" disabled={!this.state.propertyInspector} className="btn-block" style={{margin: "2px", backgroundColor: "peru"}}>property Inspector Dashboard</Button>
                        </div>
                    </div>
                </div>
            );
