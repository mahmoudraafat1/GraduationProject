  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      for (var i = 1; i < count + 1; i++) {
        ...
        rowsSt.push(<ContractData contract="property" method="getStatus" methodArgs={[i, { from: "0xa42A8B478E5e010609725C2d5A8fe6c0C4A939cB" }]} />);
      ...
      // rowsIpfs.push((<ContractData contract="property" method="getImage"  methodArgs={[i, { from: "0xa42A8B478E5e010609725C2d5A8fe6c0C4A939cB" }]} />));
      }
      

      for (var i = 1; i < count + 1; i++) {
        var propertyImg = await this.state.propertyInstance.methods.getImage(i).call();
        rowsIpfs.push(propertyImg)
        var document = await this.state.propertyInstance.methods.getDocument(i).call();
        rowsDocs.push(document);
        // row.push(<> <Col xs="6"><Card style={{textAlign: "center"}}>
        //   <CardHeader><CardTitle><h2>property {i}</h2></CardTitle></CardHeader>
        // <CardBody><div><img src={`https://ipfs.io/ipfs/${propertyImg}`} alt="" width="90%" height="90%" style={{marginBottom:"10px"}}/><p>Area: {rowsArea[i-1]}</p><p>City: {rowsCity[i-1]}</p><p>State: {rowsState[i-1]}</p><p>PID: {rowsPID[i-1]}</p><p>Price: {rowsPrice[i-1]}</p> 
        ...
        <div class="post-module">
          ...
          <div class="thumbnail">
            <div class="date">
            <div class="day">{i}</div>
            </div><img src={`https://ipfs.io/ipfs/${propertyImg}`}/>
          </div>
      ...
      <div class="post-meta"><span class="timestamp">View Verified property  <a href={`https://ipfs.io/ipfs/${document}`} target="_blank">Document</a></span></div>
          </div>
        </div>
      </Col>)
      }
