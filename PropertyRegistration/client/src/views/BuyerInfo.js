...
function sendMail(email, name){
    ...
    var tempParams = {
        from_name: email,
        to_name: name,
        function: 'request and buy any property/property',
    };
...
class BuyerInfo extends Component {
    ...
    verifyBuyer = (item) => async () => {
        ...
        await this.state.propertyInstance.methods.verifyBuyer(
            item
        ).send({
    ...
    NotverifyBuyer = (item, email, name) => async() => {
        ...
        await this.state.propertyInstance.methods.rejectBuyer(
            item
        ).send({
            from: this.state.account,
            gas: 2100000
    ...
    componentDidMount = async () => {
        ...
        try {
            ...
            var buyersCount = await this.state.propertyInstance.methods.getBuyersCount().call();
            console.log(buyersCount);
           

            var buyersMap = [];
            buyersMap = await this.state.propertyInstance.methods.getBuyer().call();
            ...
            for (let i = 0; i < buyersCount; i++) {
                // var i =3;
                var buyer = await this.state.propertyInstance.methods.getBuyerDetails(buyersMap[i]).call();

                var buyer_verify = await this.state.propertyInstance.methods.isVerified(buyersMap[i]).call();
                console.log(buyer_verify);
                buyer.verified = buyer_verify;
                
                var not_verify = await this.state.propertyInstance.methods.isRejected(buyersMap[i]).call();
