...
class ApproveRequest extends Component {
    ...
    componentDidMount = async () => {
        ...
        try{
            ...
            var requestsCount = await this.state.propertyInstance.methods.getRequestsCount().call();
            console.log(requestsCount);
            var verified = await this.state.propertyInstance.methods.ispropertyInspector(currentAddress).call();
            //console.log(verified);
            this.setState({ verified: verified });
            // var requestsMap = [];
            // requestsMap = await this.state.propertyInstance.methods.getAllRequests().call();

            for(let i = 1; i<requestsCount+1; i++){
                var request = await this.state.propertyInstance.methods.getRequestDetails(i).call();
                console.log(request);
                // console.log(request[0].toLowerCase());
                // console.log(currentAddress);
                var isPaid = await this.state.propertyInstance.methods.isPaid(request[2]).call();
                console.log(isPaid);
                requestTable.push(<tr><td>{i}</td><td>{request[0]}</td><td>{request[1]}</td><td>{request[2]}</td><td>{request[3].toString()}</td>
                <td>
                    <Button onClick={this.propertyTransfer(i, request[1])} disabled={!isPaid} className="button-vote">
    ...
    render() {
        ...
        return (
            ...
            <Card>
                    <CardHeader>
                      <CardTitle tag="h4">propertys Tranfer Request Info</CardTitle>
                    </CardHeader>
                    <CardBody>
