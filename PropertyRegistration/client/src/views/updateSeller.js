...
class updateSeller extends Component {
    ...
    componentDidMount = async () => {
        ...
        try{
            ...
            var seller_verify = await this.state.propertyInstance.methods.isVerified(currentAddress).call();
            console.log(seller_verify);
                
            var not_verify = await this.state.propertyInstance.methods.isRejected(currentAddress).call();
            ...
            seller = await this.state.propertyInstance.methods.getSellerDetails(currentAddress).call();
            console.log(seller);
            console.log(seller[0]);
            this.setState({name: seller[0], age: seller[1], aadharNumber: seller[2], panNumber: seller[3], propertysOwned: seller[4]});
            //sellerTable.push(<div><p>Name: {seller[0]}</p><p>Age: {seller[1]}</p><p>Aadhar Number: {seller[2]}</p><p>Pan Number: {seller[3]}</p><p>Owned propertys: {seller[4]}</p></div>);
              sellerTable.push(
              <Row>
                <Col md="12">
                  <FormGroup>
    ...
    updateSeller = async () => {
        if (this.state.name == '' || this.state.age == '' || this.state.aadharNumber == '' || this.state.panNumber == '' || this.state.propertysOwned == '') {
            ...
            await this.state.propertyInstance.methods.updateSeller(
                this.state.name,
                this.state.age,
                this.state.aadharNumber,
                this.state.panNumber,
                this.state.propertysOwned, 
                )
                .send({
    ...
    updateOwnedpropertys = event => (
        this.setState({ propertysOwned: event.target.value })
    )
                                    ...
                                    <CardBody>
                                        <Form>
                                            ...
                                            <Row>
                                                <Col md="12">
                                                    <FormGroup>
                                                        <label>Owned propertys</label>
                                                        <Input
                                                            type="text"
                                                            value={this.state.propertysOwned}
                                                            onChange={this.updateOwnedpropertys}
                                                        />
                                                    </FormGroup>
                                                </Col>
                                            </Row>
