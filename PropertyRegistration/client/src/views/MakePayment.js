...
class Dashboard extends Component {
  ...
  makePayment = (seller_address, amount, property_id) => async () => {
    // alert(amount);

    amount = amount*0.0000057;
    alert(amount);
    await this.state.propertyInstance.methods.payment(
      seller_address,
      property_id
    ).send({
  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      for (var i = 0; i < count; i++) {
        var paid = await this.state.propertyInstance.methods.isPaid(i + 1).call();
        var price = await this.state.propertyInstance.methods.getPrice(i + 1).call();
            ...
            <LoadingContainer>
              <Row>
                <Col lg="12" md="12">
                  <Card>
                    <CardHeader>
                      <CardTitle tag="h4">Payment for propertys<span className="duration">₹ 1 = 0.0000057 Ether</span></CardTitle>
